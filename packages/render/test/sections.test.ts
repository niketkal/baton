...
describe('sectionAcceptanceCriteria', () => {
  ...
  it('marks met criteria with [x]', () => {
    const result = sectionAcceptanceCriteria([
      { id: 'ac-1', text: 'Tests pass', status: 'met', required: true, source: 'user', provenance_refs: [] },
    ]);
  ...
  it('marks unmet criteria with [ ]', () => {
    const result = sectionAcceptanceCriteria([
      { id: 'ac-1', text: 'Tests pass', status: 'unmet', required: true, source: 'user', provenance_refs: [] },
    ]);
  ...
  it('appends _(optional)_ for non-required criteria', () => {
    const result = sectionAcceptanceCriteria([
      { id: 'ac-1', text: 'Nice to have', status: 'unmet', required: false, source: 'user', provenance_refs: [] },
    ]);
...
describe('sectionConstraints', () => {
  ...
  it('renders severity and text', () => {
    const result = sectionConstraints([
      { id: 'c-1', type: 'policy', text: 'No breaking changes', severity: 'error', source: 'user', provenance_refs: ['x'] },
    ]);
...
describe('sectionContextItems', () => {
  ...
  it('returns empty string when limit is 0', () => {
    const items = [{ kind: 'file' as const, ref: 'src/a.ts', reason: 'Key file', priority: 1, freshness_score: 1, provenance_refs: [] }];
    expect(sectionContextItems(items, 0)).toBe('');
  });

  it('sorts by priority ascending', () => {
    const items = [
      { kind: 'file' as const, ref: 'b.ts', reason: 'B', priority: 2, freshness_score: 1, provenance_refs: [] },
      { kind: 'file' as const, ref: 'a.ts', reason: 'A', priority: 1, freshness_score: 1, provenance_refs: [] },
    ];
  ...
  it('escapes pipe characters in ref and reason', () => {
    const items = [
      { kind: 'file' as const, ref: 'a|b.ts', reason: 'has|pipe', priority: 1, freshness_score: 1, provenance_refs: [] },
    ];
    const result = sectionContextItems(items);
...
describe('sectionAttempts', () => {
  ...
  it('renders attempt with tool and result', () => {
    const result = sectionAttempts([
      { id: 'a-1', tool: 'claude-code', summary: 'Tried X', result: 'partial', failure_reason: null, artifact_refs: [], created_at: '2026-01-01T00:00:00Z' },
    ]);
  ...
  it('includes failure reason when present', () => {
    const result = sectionAttempts([
      { id: 'a-1', tool: 'claude-code', summary: 'Tried X', result: 'failed', failure_reason: 'timeout', artifact_refs: [], created_at: '2026-01-01T00:00:00Z' },
    ]);
...
describe('sectionRepo', () => {
  it('returns empty string when not attached', () => {
    const result = sectionRepo({ attached: false, root: null, vcs: 'none', branch: null, base_branch: null, commit: null, base_commit: null, dirty: false });
    expect(result).toBe('');
  });

  it('renders branch, base, commit, and state when attached', () => {
    const result = sectionRepo({ attached: true, root: '/repo', vcs: 'git', branch: 'main', base_branch: 'main', commit: 'abc12345def', base_commit: 'def', dirty: false });
  ...
  it('shows Dirty when repo is dirty', () => {
    const result = sectionRepo({ attached: true, root: '/repo', vcs: 'git', branch: 'feat', base_branch: 'main', commit: 'abc1234', base_commit: null, dirty: true });
...
describe('sectionProvenance', () => {
  ...
  it('renders field_name, source_type, ref, excerpt', () => {
    const result = sectionProvenance([
      { id: 'p-1', field_name: 'objective', artifact_id: 'a-1', source_type: 'transcript', ref: 'transcript.md', span_start: null, span_end: null, excerpt: 'some text' },
    ]);
